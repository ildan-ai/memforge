...
21. (v0.5.0+; restated platform-agnostically in v0.5.2) `~/.memforge/operator-identity.yaml`, `~/.memforge/recovery-secret.bin`, `<memory-root>/.memforge/sender-sequence/<sender-uid>.yaml`, and `<memory-root>/.memforge/agent-sessions/<agent-session-id>.yaml` MUST be filesystem-restricted to the current owner: read/write for the owner only, no access for other principals on the host. Parent directories MUST be restricted to current-owner rwx-only. POSIX implementations enforce via mode 0600 / 0700 + `stat()` uid match against effective uid; Windows implementations enforce via NTFS ACLs (no inherited ACEs; no Everyone / Authenticated Users / Guests / similar principals). Adapters MUST verify at startup; fail-closed on any deviation.
