...
| 0.8.0   | v0.8.0 | 2026-06-27          | Minor (package): wikilink rewriting in memory-link-rewriter rename/rename-batch (renames no longer orphan inbound [[wikilinks]]; alias-set false-rewrite guard; cross-root disambiguation; idempotent; every rewrite logged), pointer-line + MEMORY.md SHOULD caps raised 150 -> 180 for descriptive filename slugs, and a memory-audit advisory warning on non-spec tier values; spec 0.6.2 (patch); GitHub release + CycloneDX SBOM + signed tag. Patch spec bump: Channel 3 (release tag) only per cadence, no new Zenodo DOI minted. |
