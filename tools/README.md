...
- `memory-recall`: v0.6 query-triggered recall reader. Given a query, prints the descriptions of the memories whose triggers match (descriptions only, never bodies; honors sensitivity / access; fail-open-empty). Reads the precompiled index built by `memory-index-gen --with-recall-index`; `--rebuild` refreshes it first. See the script's `--help` and `examples/recall/`.

A full reference for every CLI tool (19 commands as of v0.6.0) lives in the project root `README.md` under "Key tools". Spec-level v0.4 enforcement guidance is in `spec/SPEC.md` §"Sensitivity enforcement (v0.4.0+)" and `docs/adapter-implementation-guide.md` §"Secure-mode sensitivity enforcement (v0.4.0+)".
